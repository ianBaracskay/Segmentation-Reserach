...
save_current_figure(f"{run_id}_input_image.png")
...
mask_generator = SamAutomaticMaskGenerator(
    sam,
    points_per_side=profile_settings["points_per_side"],
    points_per_batch=sam_points_per_batch,
    pred_iou_thresh=profile_settings["pred_iou_thresh"],
    stability_score_thresh=profile_settings["stability_score_thresh"],
    box_nms_thresh=profile_settings["box_nms_thresh"],
    crop_n_layers=profile_settings["crop_n_layers"],
    crop_n_points_downscale_factor=profile_settings["crop_n_points_downscale_factor"],
    min_mask_region_area=profile_settings["min_mask_region_area"],
)
masks = generate_masks_memory_safe(mask_generator, img)
if len(masks) > max_masks_for_clip_scoring:
    print(
        f"[INFO] Pruning masks before CLIP scoring: {len(masks)} -> {max_masks_for_clip_scoring}"
    )
    masks = prune_masks_by_quality(masks, max_masks_for_clip_scoring)
...
plt.figure(figsize=(10, 10))
plt.imshow(img)
plt.imshow(sidewalk_overlay)
plt.axis("off")
plt.title(f"SAM + CLIP Sidewalk Masks: '{sidewalk_prompt}'")
save_current_figure(f"{run_id}_sidewalk_masks.png")

plt.figure(figsize=(10, 10))
plt.imshow(img)
plt.imshow(sitting_overlay)
plt.axis("off")
plt.title(f"SAM + CLIP Sitting-Area Masks: '{sitting_prompt}'")
save_current_figure(f"{run_id}_sitting_area_masks.png")
...
save_current_figure(f"{run_id}_combined_masks.png")
...
if loaded_extent_m is not None:
    ...
    plt.figure(figsize=(10, 10))
    plt.imshow(img)
    plt.imshow(amenity_heatmap, cmap="Reds", alpha=alpha_map, vmin=0.0, vmax=1.0)
    plt.axis("off")
    plt.title("Amenity Heatmap (Red = Higher Amenity Coverage)")
    save_current_figure(f"{run_id}_amenity_heatmap.png")
else:
